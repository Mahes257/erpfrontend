...
import { PAGE_SIZE_OPTIONS } from '../../utils/clientConstants';
import { getCellValue } from '../../utils/clientHelpers';
...
function TableRow({
      ...
      <td key={col.key} style={style} className={`px-4 py-3 border-b border-slate-100 ${cellClass} ${alignClass}`}>
        {col.key === 'actions' ? (
          <ActionMenu
            ...
            align="right"
            ariaLabel={`Actions for ${row.name || 'client'}`}
          />
        ) : renderers[col.key] ? (
  ...
  return (
    <tr className={`group transition-colors ${isChecked ? 'bg-[#E8F0EE]' : zebraBg} hover:bg-slate-100 dark:hover:bg-slate-100`}>
      {cells}
    </tr>
  );
}
  ...
  const renderEmpty = () => (
    <tr>
      <td colSpan={colCount} className="p-0">
        {emptyState || (
          <div className="py-14 text-center text-slate-400 font-medium">{emptyMessage}</div>
        )}
      </td>
    </tr>
  );
